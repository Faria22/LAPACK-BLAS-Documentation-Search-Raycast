...
```fortran
CGBRFS(TRANS, N, KL, KU, NRHS, AB, LDAB, AFB, LDAFB,
...
AB is COMPLEX array, dimension (LDAB,N) The original band matrix A, stored in rows 1 to KL+KU+1. The j-th column of A is stored in the j-th column of the array AB as follows: AB(ku+1+i-j,j) = A(i,j) for max(1,j-ku)<=i<=min(n,j+kl).
...
IPIV is INTEGER array, dimension (N) The pivot indices from CGBTRF; for 1<=i<=N, row i of the matrix was interchanged with row IPIV(i).
...
X is COMPLEX array, dimension (LDX,NRHS) On entry, the solution matrix X, as computed by CGBTRS. On exit, the improved solution matrix X.
