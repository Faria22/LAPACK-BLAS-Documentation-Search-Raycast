...
```fortran
ZLA_GERFSX_EXTENDED(PREC_TYPE, TRANS_TYPE, N, NRHS, A,
...
Y is COMPLEX*16 array, dimension (LDY,NRHS) On entry, the solution matrix X, as computed by ZGETRS. On exit, the improved solution matrix Y.
...
INFO is INTEGER = 0: Successful exit. < 0: if INFO = -i, the ith argument to ZGETRS had an illegal value
