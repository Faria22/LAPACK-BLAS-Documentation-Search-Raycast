...
```fortran
ZHESV_AA(UPLO, N, NRHS, A, LDA, IPIV, B, LDB, WORK,
*                            LWORK, INFO)
```
...
LWORK is INTEGER The length of WORK. LWORK >= MAX(1,2*N,3*N-2), and for best performance LWORK >= max(1,N*NB), where NB is the optimal blocksize for ZHETRF_AA. If LWORK = -1, then a workspace query is assumed; the routine only calculates the optimal size of the WORK array, returns this value as the first entry of the WORK array, and no error message related to LWORK is issued by XERBLA.
