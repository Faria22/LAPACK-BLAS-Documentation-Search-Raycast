...
```fortran
CPBRFS(UPLO, N, KD, NRHS, AB, LDAB, AFB, LDAFB, B,
...
AFB is COMPLEX array, dimension (LDAFB,N) The triangular factor U or L from the Cholesky factorization A = U**H*U or A = L*L**H of the band matrix A as computed by CPBTRF, in the same storage format as A (see AB).
...
X is COMPLEX array, dimension (LDX,NRHS) On entry, the solution matrix X, as computed by CPBTRS. On exit, the improved solution matrix X.
