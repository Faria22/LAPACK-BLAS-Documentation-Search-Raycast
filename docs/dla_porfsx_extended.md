...
```fortran
DLA_PORFSX_EXTENDED(PREC_TYPE, UPLO, N, NRHS, A, LDA,
...
Y is DOUBLE PRECISION array, dimension (LDY,NRHS) On entry, the solution matrix X, as computed by DPOTRS. On exit, the improved solution matrix Y.
...
INFO is INTEGER = 0: Successful exit. < 0: if INFO = -i, the ith argument to DPOTRS had an illegal value
