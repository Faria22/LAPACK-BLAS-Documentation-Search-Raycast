...
```fortran
CLA_GBRFSX_EXTENDED(PREC_TYPE, TRANS_TYPE, N, KL, KU,
...
AB is COMPLEX array, dimension (LDAB,N) On entry, the N-by-N matrix AB.
...
AFB is COMPLEX array, dimension (LDAF,N) The factors L and U from the factorization A = P*L*U as computed by CGBTRF.
...
Y is COMPLEX array, dimension (LDY,NRHS) On entry, the solution matrix X, as computed by CGBTRS. On exit, the improved solution matrix Y.
