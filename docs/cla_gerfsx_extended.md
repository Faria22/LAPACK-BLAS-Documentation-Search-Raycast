...
```fortran
CLA_GERFSX_EXTENDED(PREC_TYPE, TRANS_TYPE, N, NRHS, A,
...
Y is COMPLEX array, dimension (LDY,NRHS) On entry, the solution matrix X, as computed by CGETRS. On exit, the improved solution matrix Y.
...
INFO is INTEGER = 0: Successful exit. < 0: if INFO = -i, the ith argument to CGETRS had an illegal value
