...
```fortran
DSYSV_AA_2STAGE(UPLO, N, NRHS, A, LDA, TB, LTB,
...
TB is DOUBLE PRECISION array, dimension (MAX(1,LTB)) On exit, details of the LU factorization of the band matrix.
...
WORK is DOUBLE PRECISION workspace of size (MAX(1,LWORK)) On exit, if INFO = 0, WORK(1) returns the optimal LWORK.
