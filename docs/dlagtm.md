...
```fortran
DLAGTM(TRANS, N, NRHS, ALPHA, DL, D, DU, X, LDX, BETA,
*                          B, LDB)
```
...
DL is DOUBLE PRECISION array, dimension (N-1) The (n-1) sub-diagonal elements of T.

### D (in)

D is DOUBLE PRECISION array, dimension (N) The diagonal elements of T.

### DU (in)

DU is DOUBLE PRECISION array, dimension (N-1) The (n-1) super-diagonal elements of T.

### X (in)

X is DOUBLE PRECISION array, dimension (LDX,NRHS) The N by NRHS matrix X.
...
B is DOUBLE PRECISION array, dimension (LDB,NRHS) On entry, the N by NRHS matrix B. On exit, B is overwritten by the matrix expression B := alpha * A * X + beta * B.
